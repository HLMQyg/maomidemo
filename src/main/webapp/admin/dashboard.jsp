    ...
    <div id="panel-comments" class="panel-wrap" style="display:none;">
        ...
        <div class="content-card">
            ...
            <div class="card-toolbar">
                ...
                <span style="font-size:13px;color:#999;" id="commentCount"></span>
            ...
            <div class="table-wrap">
                ...
                <table>
                    ...
                    <tr><th>ID</th><th>所属帖子</th><th>评论者</th><th>内容</th><th>时间</th><th>操作</th></tr>
...
<div class="modal-overlay" id="forumDetailModal">
    ...
    <div class="modal-box" style="width:680px;">
        ...
        <button class="modal-close" onclick="closeForumDetail()">&times;</button>

        <h3 id="forumDetailTitle" style="margin-bottom:8px;"></h3>

        <div style="color:#999;font-size:12px;margin-bottom:16px;">

            <span id="forumDetailAuthor"></span> &middot;

            <span id="forumDetailCat"></span> &middot;

            <span id="forumDetailTime"></span>

        </div>

        <div id="forumDetailContent" style="white-space:pre-wrap;line-height:1.8;color:#333;margin-bottom:12px;max-height:200px;overflow-y:auto;"></div>

        <div id="forumDetailImage" style="margin-bottom:12px;display:none;"></div>

        <div style="color:#999;font-size:13px;margin-bottom:16px;">

            &#x2764; <span id="forumDetailLikes"></span> &nbsp;

            &#x1f4ac; <span id="forumDetailCommentCnt"></span> &nbsp;

            &#x1f441; <span id="forumDetailViews"></span>

        </div>

        <h4 style="margin-bottom:10px;color:#333;">评论列表</h4>

        <div id="forumDetailComments" style="max-height:300px;overflow-y:auto;"></div>
...
<script>
    ...
    function renderCommentTable() {
        ...
        if (_comments.length === 0) {
            ...
            _comments.forEach(function(c) {
                var brief = (c.content||'');
                var display = brief.length > 40 ? brief.substring(0,40) + '\u2026' : brief;
                h += '<tr>' +
                    '<td>' + c.id + '</td>' +
                    '<td style="max-width:160px;" title="' + escAttr(c.threadTitle||'') + '">' + escHtml((c.threadTitle||'').length > 18 ? (c.threadTitle||'').substring(0,18)+'\u2026' : (c.threadTitle||'')) + '</td>' +
                    '<td>' + (c.userId||'') + '</td>' +
                    '<td style="max-width:260px;"><span class="comment-brief">' + escHtml(display) + '</span>' +
                    (brief.length > 40 ? ' <button class="btn btn-sm btn-outline" onclick="showFullComment(\'' + escAttr(brief) + '\')">展开</button>' : '') +
                    '</td>' +
        ...
        document.getElementById('commentCount').textContent = '共 ' + _comments.length + ' 条';
    }

    function showFullComment(text) {
        var overlay = document.createElement('div');
        overlay.className = 'modal-overlay show';
        overlay.id = 'fullCommentOverlay';
        overlay.onclick = function(e) { if (e.target === overlay) closeFullComment(); };
        overlay.innerHTML = '<div class="modal-box" style="width:520px;max-height:70vh;">' +
            '<button class="modal-close" onclick="closeFullComment()">&times;</button>' +
            '<h3 style="margin-bottom:12px;">评论详情</h3>' +
            '<div style="white-space:pre-wrap;line-height:1.8;color:#333;max-height:50vh;overflow-y:auto;">' + escHtml(text) + '</div>' +
            '</div>';
        document.body.appendChild(overlay);
    }

    function closeFullComment() {
        var el = document.getElementById('fullCommentOverlay');
        if (el) el.remove();
    }
    ...
    function renderForumTable() {
        ...
        if(_forumThreads.length===0){h='<tr class="empty-row"><td colspan="9">暂无帖子</td></tr>'}
            ...
            h+='<tr><td>'+t.id+'</td><td><strong>'+(t.title||'')+'</strong></td><td>'+(t.userId||'')+'</td><td>'+(t.category||'')+'</td>'+
                ...
                '<button class="btn btn-sm btn-outline" onclick="viewForumDetail('+t.id+')" style="margin-left:4px;">查看详情</button>'+
    ...
    function viewForumDetail(id) {
        ...
        fetch(ctx + '/adminForum?type=detail&id=' + id)
            ...
            .then(function(d) {
                ...
                document.getElementById('forumDetailTitle').textContent = d.title || '';

                document.getElementById('forumDetailAuthor').textContent = '作者：' + (d.userId || '');

                document.getElementById('forumDetailCat').textContent = d.category || '';

                document.getElementById('forumDetailTime').textContent = d.createdAt ? d.createdAt.substring(0,16) : '';

                document.getElementById('forumDetailContent').textContent = d.content || '';

                document.getElementById('forumDetailLikes').textContent = d.likeCount || 0;

                document.getElementById('forumDetailCommentCnt').textContent = d.commentCount || 0;

                document.getElementById('forumDetailViews').textContent = d.viewCount || 0;

                var imgDiv = document.getElementById('forumDetailImage');

                if (d.imagePath) {

                    imgDiv.style.display = 'block';

                    imgDiv.innerHTML = '<img src="' + ctx + '/' + d.imagePath + '" style="max-width:100%;max-height:300px;border-radius:8px;">';
                ...
                var commentsDiv = document.getElementById('forumDetailComments');

                if (d.comments && d.comments.length > 0) {

                    commentsDiv.innerHTML = buildCommentHtml(d.comments, 0);

                } else {

                    commentsDiv.innerHTML = '<div style="color:#bbb;text-align:center;padding:20px;">暂无评论</div>';
    ...
    function buildCommentHtml(comments, depth) {
        ...
        comments.forEach(function(c) {
            ...
            h += '<div style="padding:8px 0;border-bottom:1px solid #f0f0f0;margin-left:' + indent + 'px;">' +

                '<strong>' + (c.userId || '') + '</strong>' +

                '<span style="color:#999;font-size:11px;margin-left:8px;">' + (c.createdAt ? c.createdAt.substring(0,16) : '') + '</span>' +

                '<div style="margin-top:4px;color:#444;">' + (c.content || '') + '</div>' +

                '<span style="color:#ccc;font-size:11px;">&#x2764; ' + (c.likeCount || 0) + '</span>' +

                '</div>';

            if (c.replies && c.replies.length > 0) {

                h += buildCommentHtml(c.replies, depth + 1);
    ...
    function closeForumDetail() {
        ...
        document.getElementById('forumDetailModal').classList.remove('show');
